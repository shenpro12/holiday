async function init() {
    //get data
    let hotel = await fetch('http://localhost:3000/map/api/gethotel').then(res => res.json())
    let destination = await fetch('http://localhost:3000/map/api/getdestination').then(res => res.json())
    let eating_spot = await fetch('http://localhost:3000/map/api/geteatingspot').then(res => res.json())
        //create maker
    ...
    document.getElementsByClassName('leaflet-routing-container')[0].classList.add('leaflet-routing-container-hide') //hide routing
        //
    function getMaker(makerDatas, iconUrl, iconSize) {
        ...
        for (let i = 0; i < makerDatas.length; i++) {
            let marker = L.marker([makerDatas[i].lat, makerDatas[i].long], { icon: icon, name: makerDatas[i].name, data: makerDatas[i] })
            ...
            marker.on('mouseover', function(e) {
                    ...
                    markerTemp.on('click', function(e) {
                        ...
                        <iFrame style="border:none;" src="http://localhost:3000/map/api/infomation?id=${marker.options.data._id}" width="100%" height="700px" allowfullscreen></iFrame>
                        </div>`);
                        sidebar.show();
